...
function Intersect($wire1, $wire2) {
    ...
    $cX = 0;
    $cY = 0;
    $intersectDistance = 100000000;
    
    $points1 = explode(',', $wire1);
    //print_r($points1);
    $points2 = explode(',', $wire2);
    // print_r($points2);

    foreach ($points1 as $point) {
        //echo "Point: $point\r\n";
        $direction = $point[0];
        $count = (int) substr($point, 1);

        if ($direction == 'R') {
            while($count > 0) {
                ++$cX;
                if (!array_key_exists($cX, $board)) $board[$cX] = array();
                $board[$cX][$cY] = 1;
                $count--;
            }
        }
        if ($direction == 'L') {
            while ($count > 0) {
                --$cX;
                if (!array_key_exists($cX-1, $board)) $board[$cX] = array();
                $board[$cX][$cY] = 1;
                $count--;
            }
        }
    ...
    foreach ($points2 as $point) {
        ...
        if ($direction == 'R') {
            while($count > 0) {
                ...
                if (!array_key_exists($cX, $board)) $board[$cX] = array();
                if ($board[$cX][$cY] == 1) {
                    echo "  Intersect $cX, $cY\r\n";
                    $intersectDistance = min($intersectDistance, abs($cY) + abs($cX));
                }
                $count--;
            }
        }
